...
public final class WikiManager
{
     ...
     */
    public boolean canCreateWiki(String userName, XWikiContext context)
    {
        /* Allow registered users (but not empty/null user names) and virtual mode wiki. */
        return userName != null && userName.trim().length() != 0
            && !XWikiRightService.GUEST_USER_FULLNAME.equals(userName) && context.getWiki().isVirtualMode();
    }

    /**
     * @param wikiName the wiki name to check
     * @param userName the user to check
     * @param context the XWiki context
     * @return true if the it's possible for the specified user to edit the specified wiki descriptor
     */
    public boolean canEditWiki(String wikiName, String userName, XWikiContext context)
    {
        try {
            XWikiServer wikiServer = getWikiAlias(wikiName, 0, true, context);
            ...
            String mainWikiPreferencesDocumentName =
                String.format(WIKI_PREFERENCES_PREFIXED_FORMAT, context.getMainXWiki());

            /* Owner or main wiki admin. */
            return wikiOwner.equals(userName)
                || rightService.hasAccessLevel(RIGHT_ADMIN, userName, mainWikiPreferencesDocumentName, context);
        } catch (Exception e) {
            if (LOG.isErrorEnabled()) {
                XWikiPluginMessageTool msg = getMessageTool(context);
                LOG.error(msg.get(WikiManagerMessageTool.LOG_MANAGERCANEDIT), e);
            }
            return false;
        }
    }
     ...
     * @param userName the user to check
     * @param context the XWiki context
     * @return true if the it's possible for the specified user to delete the specified wiki
     */
    public boolean canDeleteWiki(String wikiName, String userName, XWikiContext context)
    {
        try {
            XWikiServer wikiServer = getWikiAlias(wikiName, 0, true, context);
            String wikiOwner = wikiServer.getOwner();
            ...
            return context.getWiki().isVirtualMode()
                && (wikiOwner.equals(userName) || rightService.hasAccessLevel(RIGHT_ADMIN, userName,
                    mainWikiPreferencesDocumentName, context));
        } catch (Exception e) {
            if (LOG.isErrorEnabled()) {
                XWikiPluginMessageTool msg = getMessageTool(context);
