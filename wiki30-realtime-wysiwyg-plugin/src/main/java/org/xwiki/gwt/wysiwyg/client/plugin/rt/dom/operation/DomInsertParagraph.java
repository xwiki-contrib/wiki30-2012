...
public class DomInsertParagraph extends AbstractDomOperation
{
    ...
    public Range execute(Document document)
    {
        ...
        Node container = domUtils.getNearestBlockContainer(targetNode); // this is a p or a heading element
        Node paragraphOrHeading = container.cloneNode(false);
        ...
        if (caretContainer == container.getNextSibling()) {
            caretContainer = paragraphOrHeading;
        }

        // Move the right side of the split to the new paragraph.
        paragraphOrHeading.appendChild(Element.as(container.getNextSibling()).extractContents());
        container.getParentNode().replaceChild(paragraphOrHeading, container.getNextSibling());

        // Make sure that both lines generated by the split can be edited.
        Element.as(container).ensureEditable();
        Element.as(paragraphOrHeading).ensureEditable();
